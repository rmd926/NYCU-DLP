        ...
        with torch.no_grad():
            ...
            for step in range(self.total_iter):
                if step == self.sweet_spot:
                    continue
                mask_schedule_ratio = (step + 1) / (self.total_iter) #ratio of mask scheduling
                
                z_indices_predict, mask_bc = self.model.inpainting(z_indices_predict, mask_bc, mask_num, mask_schedule_ratio)#
...
if __name__ == '__main__':
    ...
    parser.add_argument('--load-transformer-ckpt-path', type=str, default='./transformer_checkpoints/best(Ep47).pth', help='load ckpt')
    parser.add_argument('--test-maskedimage-path', type=str, default='C:/Users/蘇柏叡/Desktop/lab5_dataset/masked_image', help='Path to testing image dataset.')
    parser.add_argument('--test-mask-path', type=str, default='C:/Users/蘇柏叡/Desktop/lab5_dataset/mask64', help='Path to testing mask dataset.')
    ...
    for i, (image, mask) in tqdm.tqdm(enumerate(zip(t.mi_ori, t.mask_ori)), total=len(t.mi_ori), desc="Inpainting Images", leave=False):
        ...
        #tqdm.tqdm.write(f"Completed inpainting for image {i+1}/{len(t.mi_ori)}")
